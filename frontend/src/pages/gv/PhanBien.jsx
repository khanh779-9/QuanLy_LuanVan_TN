    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className={`mt-4 text-4xl font-extrabold ${currentTone.value}`}>
            {value}
          </p>
        </div>
...
export default function GVPBPhanBienPage() {
  ...
  const scoredItems = tableData.filter(
    (deTai) => deTai.diemPhanBien !== undefined && deTai.diemPhanBien !== null,
  );
  ...
  const averageScore =
    daChamCount > 0
      ? scoredItems.reduce(
          (sum, deTai) => sum + Number(deTai.diemPhanBien || 0),
          0,
        ) / daChamCount
      : 0;
  const completionRate =
    tableData.length > 0
      ? Math.round((daChamCount / tableData.length) * 100)
      : 0;
  const reviewSignals = { baoVe: 0, boSung: 0, khong: 0 };

  scoredItems.forEach((deTai) => {
    const students = Array.isArray(deTai?.data_json?.gvpb?.sinh_viens)
      ? deTai.data_json.gvpb.sinh_viens
      : [];
    students.forEach((sv) => {
  ...
  function openEdit(deTai) {
    ...
    const gvpb =
      deTai.data_json && deTai.data_json.gvpb ? deTai.data_json.gvpb : {};
        ...
        <div className="relative z-10 flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-2xl">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-600 md:text-base">
              Quản lý phản hồi phản biện, rà soát chất lượng đề tài và nhập các
              đề nghị điều chỉnh một cách tập trung hơn.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1.2fr_1fr]">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Mức độ hoàn thành nhập điểm và nhận xét phản biện trong đợt này.
              </p>
            </div>
          ...
          <div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-3">
            ...
            <div className="rounded-2xl bg-amber-50 px-4 py-4">
              ...
              <p className="mt-2 text-3xl font-bold text-amber-700">
                {daChamCount > 0 ? averageScore.toFixed(1) : "0.0"}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-2 text-sm text-slate-500">
            Tổng hợp nhanh đề nghị của giảng viên phản biện dành cho sinh viên
            sau khi đánh giá.
          </p>
          <div className="mt-5 space-y-3">
            <FeedbackRow
              label="Được bảo vệ"
              value={reviewSignals.baoVe}
              tone="violet"
            />
            <FeedbackRow
              label="Bổ sung / hiệu chỉnh"
              value={reviewSignals.boSung}
              tone="amber"
            />
            <FeedbackRow
              label="Không được bảo vệ"
              value={reviewSignals.khong}
              tone="rose"
            />
          </div>
        </div>
          ...
          <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-sm text-slate-500">
                Theo dõi phản hồi, điểm và thao tác nhập phiếu cho từng đề tài.
              </p>
            </div>
          ...
          <table className="w-full border-separate border-spacing-0">
            <thead>
              <tr className="bg-slate-50">
                <th className="px-4 py-3 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-200">
                  Mã
                </th>
                <th className="px-4 py-3 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-l border-slate-200">
                  Tên đề tài
                </th>
                <th className="px-4 py-3 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-l border-slate-200">
                  Sinh viên
            ...
            <tbody>
              {isLoading ? (
                [...Array(5)].map((_, i) => (
                  <tr key={i}>
                    {[...Array(6)].map((_, ci) => (
                      <td
                        key={ci}
                        className={`px-4 py-3 border-t border-slate-100 ${ci > 0 ? "border-l border-slate-100" : ""}`}
                      >
                ...
                tableData.map((deTai) => {
                  const hasScore =
                    deTai.diemPhanBien !== undefined &&
                    deTai.diemPhanBien !== null;
                  return (
                    <tr
                      key={deTai.maDeTai}
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-sm text-slate-700 border-t border-l border-slate-100 align-middle text-center">
                        {Array.isArray(deTai.sinh_viens) &&
                        deTai.sinh_viens.length > 0 ? (
        ...
        >
          <div className="max-h-[70vh] overflow-y-auto pr-0">
            ...
            {(() => {
              ...
              const sinhViens = Array.isArray(
                editDeTai.data_json?.gvpb?.sinh_viens,
              )
                ? editDeTai.data_json.gvpb.sinh_viens
                : Array.isArray(editDeTai.sinh_viens)
                  ? editDeTai.sinh_viens
                  : [];
                    ...
                    {sinhViens.map((sv, idx) => (
                      ...
                      >
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2">
                          <div>
                            ...
                            <input
                              ...
                              max="10"
                              step="0.5"
                              className="w-full border border-slate-200 rounded px-2 py-1 text-xs bg-white focus:outline-none focus:border-blue-400 placeholder:text-slate-300"
                              placeholder="0.0"
                              value={editForm.diemPhanTich[idx] ?? ""}
                              onChange={(e) =>
                                setEditForm((f) => {
                                  const arr = [...f.diemPhanTich];
                                  arr[idx] = e.target.value;
                                  return { ...f, diemPhanTich: arr };
                                })
                              }
                            />
                          </div>
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setEditForm((f) => {
                                  const arr = [...f.diemThietKe];
                                  arr[idx] = e.target.value;
                                  return { ...f, diemThietKe: arr };
                                })
                              }
                            />
                          </div>
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setEditForm((f) => {
                                  const arr = [...f.diemHienThuc];
                                  arr[idx] = e.target.value;
                                  return { ...f, diemHienThuc: arr };
                                })
                              }
                            />
                          </div>
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setEditForm((f) => {
                                  const arr = [...f.diemBaoCao];
                                  arr[idx] = e.target.value;
                                  return { ...f, diemBaoCao: arr };
                                })
                              }
                            />
                          </div>
                          <div>
                            ...
                            <input
                              ...
                              max="40"
                              step="0.5"
                              className="w-full border border-slate-200 rounded px-2 py-1 text-xs bg-white focus:outline-none focus:border-blue-400 font-medium placeholder:text-slate-300"
                              placeholder="0.0"
                              value={editForm.diemTongCong[idx] ?? ""}
                              onChange={(e) =>
                                setEditForm((f) => {
                                  const arr = [...f.diemTongCong];
                                  arr[idx] = e.target.value;
                                  return { ...f, diemTongCong: arr };
                                })
                              }
                            />
                          </div>
                        ...
                        <div className="mt-2 flex items-center gap-2">
                          ...
                          <select
                            className="w-full md:w-auto min-w-[120px] border border-slate-200 rounded px-2 py-1 text-xs bg-white focus:outline-none focus:border-blue-400"
                          ...
                          >
                            ...
                            <option value="Không được bảo vệ">
                              Không được bảo vệ
                            </option>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                <div>
                  ...
                  <textarea
                    ...
                    value={editForm.uuDiem}
                    onChange={(e) =>
                      setEditForm((f) => ({ ...f, uuDiem: e.target.value }))
                    }
                  />
                </div>
                <div>
                  ...
                  <textarea
                    rows={2}
                    className="w-full border border-slate-200 rounded px-2 py-1 text-xs bg-white focus:outline-none focus:border-blue-400 placeholder:text-slate-300"
                    placeholder="Nhập thiếu sót..."
                    value={editForm.thieuSot}
                    onChange={(e) =>
                      setEditForm((f) => ({ ...f, thieuSot: e.target.value }))
                    }
                  />
                </div>
                <div className="md:col-span-2">
                  ...
                  <textarea
                    ...
                    value={editForm.ndDieuChinh}
                    onChange={(e) =>
                      setEditForm((f) => ({
                        ...f,
                        ndDieuChinh: e.target.value,
                      }))
                    }
                  />
                </div>
                <div className="md:col-span-2">
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setEditForm((f) => ({ ...f, cauHoi: e.target.value }))
                    }
                  />
                </div>
                <div className="md:col-span-2">
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setEditForm((f) => ({ ...f, nhanXet: e.target.value }))
                    }
                  />
                </div>
                <div className="md:col-span-2 mt-2 grid grid-cols-1 gap-3 md:grid-cols-[minmax(0,220px)_1fr]">
                  <div className="flex flex-col md:flex-row md:items-end gap-2">
                    <label className="text-xs font-bold text-slate-700 whitespace-nowrap md:pb-2">
                      Kết luận Thuyết minh:
                    </label>
                    <select
                      className="w-full md:w-40 border border-slate-200 rounded px-2 py-2 text-sm bg-white focus:outline-none focus:border-blue-400 font-medium"
                      value={editForm.thuyetMinh}
                      onChange={(e) =>
                        setEditForm((f) => ({
                          ...f,
                          thuyetMinh: e.target.value,
                        }))
                      }
                    >
            ...
            <button
              ...
              onClick={async () => {
                if (editDeTai?.maDeTai) {
                  const { exportWordGVPB } =
                    await import("../../services/deTaiService");
                    ...
                    data: {
                      ...
                      data_json: {
                        gvpb: {
                          ...
                          cauHoi: editForm.cauHoi,
                          thuyetMinh: editForm.thuyetMinh,
                          sinh_viens: Array.isArray(editDeTai.sinh_viens)
                        ...
                        gvhd:
                          editDeTai.data_json && editDeTai.data_json.gvhd
                            ? editDeTai.data_json.gvhd
                            : {
                                tong_diem: "",
                                nhanXet: "",
                                uuDiem: "",
          ...
          {updateMut.isError && (
            <div className="text-red-500 mt-3 text-xs text-center font-medium bg-white p-2 rounded border border-red-100">
              Cập nhật thất bại, vui lòng kiểm tra lại dữ liệu và thử lại.
            </div>
          )}
        </Modal>
      )}
